...
// pre: the `ordinals` span outlives the ingest call; the engine copies what it keeps.
...
TEST(OrdinalHistogramTest, BinIndexIsLog2OctaveClampedToB)
{
    ...
    EXPECT_EQ(meta::ordinal_bin_index(OrdinalSchedule::DurationLog2Ns, 3), 1u);
...
TEST(OrdinalHistogramTest, AccumulatesBinnedCountsOnTheRightTemplate)
{
    ...
    EXPECT_EQ(hist.schedule_id, "dur-log2-ns-v1");
    EXPECT_EQ(hist.total, 8u);
    EXPECT_EQ(hist.counts.size(), 48u);
...
TEST(OrdinalHistogramTest, MultipleFieldsEmittedInSortedOrder)
{
    ...
    EXPECT_EQ(hists[0].field_name, "latency_ms");
...
TEST(OrdinalHistogramTest, DiffPairsOrdinalHistogramsBothSides)
{
    ...
    const auto baseline{build(100)};
    const auto changed{build(100'000)};
    ...
    EXPECT_EQ(delta.current_schedule_id, "dur-log2-ns-v1");
