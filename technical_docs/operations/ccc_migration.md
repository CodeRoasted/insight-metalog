...
(`clang-format off`/`on`, the namespace closer), the `wallclock` check module and
...
question with two repos in it. The comment now states the truth rather than *"Always present."*
  ...
  DISCHARGED 2026-09-12 `coderoast ad5509cc` for Daidalos's mechanism half (W20), ruled at `DN-100.D1` — compose does NOT merge the acquisition block, and the absent block stays the composed document's state. Of the block's nine members, two are exact functions of the inputs (`records_with_component`, `span_records`), `closed_cells` is recomputable only from the composed cube, and six count distinct values or unresolved span edges, where the inputs bound the composed value (`max(a, b) ≤ n ≤ a + b`) and never determine it. A merge would therefore publish bounds under the names of observations, and a block holding only the exact members would read as assessed while `where_admissible` still answers no. No code is owed: `compose.cpp` has zero `acquisition` tokens, `diff_engine.cpp`'s `where_admissible` and `span_context` already read the absence as inadmissible, and no composed document reaches either at HEAD (the production `compose()` callers are detection's pyramid, which reads no acquisition member, and `ShardedPipeline`, used only by its own test). The shape that would re-open it, for a composed baseline that needs WHERE, is a distinct-component SET bounded by `detection::kDimensionCardinalityCap` plus a saturation flag, which merges exactly — never a count, and never a conjunction of the children's verdicts, since two children under the cap can union past it. `LEXICON.md`'s acquisition-block row is corrected in the same commit: it named `burstiness` and a sampling-reliability fact, neither of which is a member (0 hits in `insight-metalog`'s `api/` and `src/`).
...
framing is unearned; both are decisions, not comments.
  ...
  PARTIAL. DISCHARGED 2026-09-12 `coderoast 79446e78` for Daidalos's design-tier half (W20) — zero design-tier sites carried the wording, so nothing was rewritten; `DN-11.D20` records the calibration instead, because `DN-11` consumes the edges twice (its `ordinal_ratio_shift` bands and the `kMinShiftDurationTicks = 16` floor derived from the NONE→LOW edge) and calls them *frozen*, which is true. Swept: `design_notes/`, `adr/`, `studies/` and `LEXICON.md` for *pre-regist* and *anti-endogam* within the W1 / octave / ordinal context, and `bibles/` also in French (*pré-enregistr*). The sweep's positive controls read correctly: `STU-3.A1` pre-registers the thin-sample floor's acceptance with the bands held as inputs, and `DN-51.D21`'s *"pre-registered falsifier"* names a witness arm, not this statistic; `ADR-31` has zero hits, so no ADR needs the Founder. The slot's rule: a gate verdict is not endogamous in these edges, because they were set on a synthetic stream disjoint from `DN-11`'s GitLab population, and re-cutting them on that population voids the verdict. Left open for Hephaïstos in this repo: the bands' `refs: STU-3.A1` names a study that consumes them, not their provenance, and the calibration coordinate (scenario 35, poles 9.96 and 0.14) now survives only in `dbd53a4`'s diff — the site owes a `note:` stating it.
  DISCHARGED 2026-09-13 `insight-metalog 29f3214` for the site this repo owed (W21) — `ordinal_w1`'s bands in `api/metalog.api.cppm` drop `refs: STU-3.A1`, a study that holds the bands fixed as inputs, and carry one `note:` (*calibrated on scenario 35, not pre-registered; a re-cut on gate data voids the gate*) with `refs: DN-11.D20`, the slot that now records the poles 9.96 and 0.14. Comment-only, so no arm can red; `malf format --check`: 74 files, 0 misformatted, 0 CCC violations. `kShiftSampleFloor`'s `refs: STU-3.A1` in `src/operations/diff.cpp` stays, because `STU-3.A1` is that floor's acceptance record.
...
its justifying measurement recorded nowhere that is owed to survive.
  ...
  DISCHARGED 2026-09-12 `insight-metalog afc8dfd` — the live witness already existed and now states the whole eviction. `ReservoirTest.ErrorClassReserveRetainsFailureAgainstNonFailureStorm` (`tests/reservoir/test_reservoir.cpp`, in the tree since `insight-metalog d70f88f`, 2026-06-28) builds the loss the reserve exists for in one deterministic window: three non-failure branches each out-score a rare `connection refused to db` Error on salience in a 3-slot reservoir behind a 3-entry top_k. With `reservoir_error_reserve = 1` the failure is retained at a LOWER salience than every branch kept; with 0 it is evicted and all three branches keep the slots. The negative control checked the reservoir only; it now also asserts the failure is outside top_k, so the arm reads: out of BOTH tiers without the reserve, retained with it. Red with the reserve disabled inside `admit_reservoir` (`reserve` forced to 0, the config left at 1): the with-reserve half fails at `reservoir_has(with_reserve, …)` (actual false), and `ErrorClassReserveIsExemptFromPerKindCap` reds beside it (1 vs > 1); restored by byte copy, `sha256sum -c` OK. What this arm does NOT carry is the original field measurement (a `testTimeout (FAILED)` lost in a live 3 508-template CI window): it witnesses the mechanism the policy ships, not the frequency of the loss in the field. `malf test insight-metalog` 308 of 313 on clang-21 and on gcc-16.2 alike, the 5 being the intended `ComposeShardFold` reds (the shard-fold defect they pin is planned work, not this line's); `malf format --check insight-metalog`: 74 files, 0 misformatted, 0 CCC violations.
...
not govern. Whether the repo wants an arm that IS in scope is a plan-tier question, not a comment.
  ...
  DISCHARGED 2026-09-12 `insight-metalog 2259589` — the in-scope arm lands and MISSES the headline, so the miss is a finding for `metalog-spec`'s owner and never a claim of this repo. `SpecEnvelopeHeadline.StatsOnlyDocumentAtTopK32InlineOverOneMillionLines` (`tests/serialization/test_spec_envelope_headline.cpp`) builds the headline's scope from the engine: one window of exactly 1 000 000 lines over 64 templates in an integer harmonic series (template i carries floor(210 000 / (i + 1)) lines, the remainder on template 0), no parameters, one level, `top_k_size` 32 (full), `reservoir_size` 0, behaviour, stability and branching off, and the cube and acquisition block — which the engine always emits — cleared; inline mode, the one `to_json` has. Measured: **6 113 bytes per million lines**, clang-21 and gcc-16.2 byte-identical (6 655 with the cube and acquisition kept), against the headline's 4 096. Decomposed on the document: the 32 inline top_k entries take 5 438 bytes (167 to 172 each, a 55-to-56-byte skeleton string included) and everything else 675. Computed on the same document rather than emitted: with every inline string emptied it is still 4 331 bytes, and with the template key dropped (id-only, which this producer cannot emit) 3 883. So inline at top_k 32 cannot meet 4 KB for any skeleton, and `SPEC.md`'s own § 11.2 per-entry costs and § 11.4 (~9 KB stats-only at top_k 64) already contradict § 11.5's two scope clauses — for the spec's owner. The arm pins the measured 6 113 as a characterization and asserts scope first (lines, a full top_k, no reservoir, behaviour or stability, 32 inline template strings). Red under a one-key serializer mutation (`"level"` to `"lvl"` on the top_k entry wire): 6 049 against 6 113; restored by byte copy, `sha256sum -c` OK. Same gates as the line above.
  PARTIAL. DISCHARGED 2026-09-13 for Eqya's half, by ruling — brought to `metalog-spec`'s owner, the Founder, as a decision, and not edited: that repository is public and governs itself. The miss is arithmetic, not tuning. Clause 1 (inline, `top_k_size` ≤ 32) measures 6 113 bytes. Clause 2 (id-only, ≤ 64) is contradicted by the spec's own § 11.4, which prices a stats-only document at `top_k_size` 64 at ~9 KB, and by § 3.6.1, which prices 64 id-only entries at ~9 KB. Options put to him: retire the number as a headline, rescope it to what an implementation meets, or change the wire to reach it. Nothing is owed in this repo: its arm pins the measurement and asserts no headline.
  DISCHARGED 2026-09-14 `insight-metalog 7a85f97` — `metalog-spec a68bf10` withdrew the headline, so this repo's arm no longer measures against it. `SpecEnvelopeHeadline.StatsOnlyDocumentAtTopK32InlineOverOneMillionLines` is renamed `StatsOnlyEnvelopeSize.TopK32InlineOverOneMillionLines` (`tests/serialization/test_stats_only_envelope_size.cpp`); `kSpecHeadlineBytes` and the headline figure in its failure message are gone, while the 6 113-byte characterization pin, the scope asserts and the stream are unchanged. `benchmarks/bench_metalog.cpp`'s note citing the 4 KB target is deleted. The arm passes on clang-21 and gcc-16.2, with `insight-metalog` at 314 of 319 on both, the 5 being the by-design `ComposeShardFold` reds.
...
* **Needs a measurement this lane could not afford.** `Cell::operator==` in
  ...
  DISCHARGED 2026-09-11 `insight-metalog d2cb1a1` — measured, and the scar is not live: with the operator flipped to `friend constexpr bool operator==(const Cell&, const Cell&) noexcept = default;`, `malf build insight-metalog --profile linux-gcc16-release` compiles `metalog.detail.cube.cppm.o` into both the library and the test binary with no diagnostic and no internal compiler error (66 C++ objects, exit 0). So the note and the hand-written body are deleted and the defaulted friend ships. `malf test insight-metalog` reads 301 of 306 on gcc-16.2 and 301 of 306 on clang-21, the five failures being the intended `ComposeShardFold` gates-before-code arms (ctest 113, 116, 117, 118, 119), unchanged in number and identity. Measured for this type only (`std::array<std::uint32_t, 4>`): `MEM:gcc-named-modules-failure-catalog` item 15 still records the shape as an ICE in general, and this measurement does not reach other types.
...
2. **`malf lint --all-files` reads 2 findings and 10 of 26 selected files are NOT CHECKED.**
   ...
   DISCHARGED 2026-09-11 for the Hephaïstos half (the two diagnostics) `insight-metalog 654f288` — `compose.cpp`'s top-n-grams cap is an `if`/`else` instead of a nested conditional, and `MetaLogEngine::build_presence_churn` is `static`, since it reads no member. Verified: `malf lint --all-files` in `insight-metalog` checks 16 translation units and reports 0 findings (its exit is 1 only for the coverage refusal on `scripts/determinism_fixture.cpp`, which has no compile command in the partial clang database and was not touched). Positive control: `clang-tidy-21 --checks='-*,readability-avoid-nested-conditional-operator'` over HEAD's `compose.cpp`, compiled with the same command, fires at line 509 and is silent on the new file. The `build_presence_churn` half has no parent control — HEAD's `const` definition no longer matches the new declaration. `malf format --check insight-metalog`: 71 of 71 files, 0 violations. `malf test insight-metalog`: 301 of 306 on both toolchains, as above.
