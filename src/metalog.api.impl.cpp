...
//      module TU (insight.detection) emits its OWN copy/move and miscompiles the
//      optional<CubeBlock> member under Release /O2 /Ob2. User-provided (non-defaulted) bodies are
//      not synthesizable, so the consumer must emit a real CALL into this single definition. (A `=
//      default` body in this unit additionally LNK2005-collides with the consumer's synthesized
//      one.)
//   2. This unit itself is compiled at /Ob1 on MSVC (CMakeLists set_source_files_properties): the
//      bug is the /Ob2 aggressive inliner mishandling optional<CubeBlock>'s _Has_value
//      (RelWithDebInfo /Ob1 never reproduced it), so the optional copy/move below is emitted
//      correctly here. The bodies are plain member-wise == the implicit/`= default` semantics — no
//      per-member dodges.
...
// // (internal_copy_keeps_empty=0) with ALL sizes matching the consumer — so it's metalog's
// codegen,
...
// // Cold-path special members; output is byte-identical (no float) — determinism goldens
// unaffected. #if defined(_MSC_VER) #pragma optimize("", off) #endif

// namespace insight::metalog
// {

// // These bodies ARE member-wise == `= default`, but MUST stay user-provided (see header): a `=
// default`
// // special member is synthesizable, which both reintroduces the consumer-side miscompile and
// collides
...
// // metalog.api.cppm (metalog_version{"0.6.0"}, the `{}`-initialized blocks, and `cube` →
// optional's
// // disengaged default) still apply since `cube` etc. are absent from this (empty) mem-init list.
// Being
// // user-provided, consumers/other TUs CALL this single definition instead of synthesizing their
// own. MetaLogDocument::MetaLogDocument() {}
...
// // Hand-written (empty), NOT `= default`: a defaulted dtor is synthesizable, so the consumer
// would
